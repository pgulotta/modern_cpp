...
//  auto student      ( make_tuple( 123456, "John Doe", 3.7 ) );
  auto student_desc ( make_tuple( "ID",   "Name", "Level",   "GPA" ) );
  auto student      ( make_tuple( 123456, "John Doe", 7, 3.7 ) );

  cout << "student_desc =" << student_desc << '\t' << "  student = " << student << '\n';

  cout << "tuple_cat = " << tuple_cat( student_desc, student ) << '\n';

  auto zipped ( zip( student_desc, student ) );
  cout << "zipped = " << zipped << '\n';

  auto numbers = {0.0, 1.0, 2.0, 3.0, 4.0};
  cout << "zip of sum_min_max_avg = "  << zip(
